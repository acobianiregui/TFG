...
\begin{document}
\input{latex/anexos/Anexo B/portadaB}
...
En este contexto, diversos estudios, como el publicado por \textbf{\textit{\cite{chen_2cfastica_2024}}}, han explorado la viabilidad de utilizar ICA para descomponer señales EMG multicanal y aislar componentes asociados a músculos individuales o grupos de unidades motoras. Estos trabajos muestran que la estructura de mezcla lineal de la sEMG (EMG no invasivo), junto con la independencia relativa entre fuentes fisiológicas, convierte a ICA en un candigtdato adecuado para mejorar la capacidad de interpretación de las grabaciones. 
...
A pesar de estas limitaciones, el interés por el análisis avanzado de EMG ha aumentado de forma notable en los últimos años. Este crecimiento se observa especialmente en áreas como la rehabilitación neuromuscular, la monitorización de la reinervación tras lesiones periféricas, el control de prótesis y el estudio de trastornos del movimiento. Muchas de estas investigaciones se inician en modelos de animales, especialmente conejos y ratas. 

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{Anexo B/figuras/EMG_rats.png}
    \caption{Esquema del estudio realizado por \textbf{\textit{\cite{tang_improving_2025}}}.}
    \label{fig:EMG_r}
\end{figure}
...
Por otro lado, experimentalmente también se emplea ampliamente  los \textbf{ Regenerative Peripheral Nerve Interfaces (RPNI)}, donde se implanta un injerto muscular denervado y libre directamente al nervio afectado. Este injerto actúa como un amplificador focalizado y, en consecuencia, genera señales EMG de calidad superior y con mínima contaminación de músculos próximos.  (\textbf{\textit{\cite{vu_regenerative_2020}}})


Sin embargo, al hacer uso de músculos auxiliares como bioamplificadores (sea RPNI o TMR), estos interaccionan con el nervio redirigido y modifican la forma y características de la señal emitida. Esto se debe a que el desvío del nervio introduce cambios en la geometría y densidad de las fuentes de las unidades motoras, afectando a la transferencia del potencial a través de tejidos y, en consecuencia, la forma temporal y espacial del potencial de acción \textit{(MUAP)}. Estas diferencias, respecto a la señal original, dificultan el procesamiento de la información del nervio y empeora la detección de activación, lo que potencialmente impacta el rendimiento de las prótesis.

\clearpage

El estudio conducido por \textbf{\textit{\cite{frost_regenerative_2018}}} entrena un modelo para la detección muscular en ratas a partir de una señal EMG rectificada para tanto músculo sano como reinervado. Como se puede observar, la figura \ref{fig:RPNI_C} muestra las áreas de detección nerviosa en un músculo de referencia o control, demostrando una identificación consistente de las activaciones. 
No obstante, en la figura \ref{fig:RPNI_O} se puede apreciar que esta misma estrategia presenta un comportamiento diferente en el caso del músculo reinervado. A pesar de estar empleando RPNI, la morfología alterada de los MUAP provoca variaciones más pronunciadas e inconsistentes en los periodos de activación y reposo. Adicionalmente, las amplitudes (en milivoltios) experimentan una atenuación variable debido a la redistribución de las fuentes motoras.


\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Anexo B/figuras/RPNI_Control.png}
    \caption{Análisis de la señal EMG de control a lo largo de 40 segundos.}
    \label{fig:RPNI_C}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.52\linewidth]{Anexo B/figuras/RPNI.png}
    \caption{Análisis de la señal EMG RPNI a lo largo de 40 segundos.}
    \label{fig:RPNI_O}
\end{figure}


Resumiendo, la señal EMG captada por los electrodos puede presentar retardos, variaciones en la amplitud y patrones de activaciones atípicas.
...
\begin{itemize}
    \item Analizar y tratar la señal del músculo reinervado.
 \end{itemize}   
    %Explicacion aqui
    \noindent Cómo se mencionó en la sección introductoria de este documento, las señales de los músculos reinervados se ven alteradas por la interacción con el músculo auxiliar y, por lo tanto, sus características son considerablemente diferentes respecto a la señal de un músculo sano. Comprender las diferencias entre ambas señales en cuanto amplitud, frecuencia, fase será un aspecto clave para poder completar el trabajo.
\begin{itemize}
    \item Separación de la señal EMG, resultando en la señal del nervio y del músculo auxiliar
\end{itemize}
\noindent Una vez analizada la señal del músculo reinervado e identificadas sus características, se puede emplear esta información para diseñar y entrenar un modelo que separe la señal . En otras palabras, se busca desarrollar un procedimiento capaz de distinguir que componentes $(S_1,S_2,S_3,.. S_n)$ pertenecen realmente al músculo reinervado y cuales provienen de músculos adyacentes.
\begin{itemize}
    \item Discusión de varios algoritmos, exponiendo sus pros y contras, y finalmente seleccionando uno para su implementación.
\end{itemize}
Como se comentará en la sección de metodología, los diferentes métodos que se considerarán en el Trabajo Fin de Grado tienen sus características particulares, pudiendo funcionar mejor o peor dadas circunstancias específicas. En consecuencia, se debe probar y analizar los algoritmos para encontrar la combinación que mejor optimice la separación. 
\begin{itemize}
    \item Probar eficacia en una base real de señales EMG.
\end{itemize}
    \noindent Por lo general, los algoritmos de separación de fuentes consiguen señales que maximicen su independencia entre si, pero en muchos casos resulta imposible recuperar las fuentes originales con una precisión perfecta.  Por ello, probablemente el objetivo más importante de este proyecto es evaluar la actuación del algoritmo desarrollado y considerar si sus resultados pueden conllevar una mejora sustancial en el manejo de prótesis. Potencialmente, si se consigue un rendimiento adecuado, se podría plantear validar el  algoritmo en los datos obtenidos del estudio de \textbf{\textit{\cite{gonzalez-prieto_regenerative_2024}}}.
\begin{itemize}
    \item Explicar el uso y comportamiento del algoritmo para el público menos técnico (ejemplo: manual de uso para profesionales sanitarios).
\end{itemize}
\noindent Los conceptos que se abordan en este Trabajo Fin de Grado, como técnicas de aprendizaje automático (\textit{Machine Learning}), estadística avanzada y procesado de señal, requieren una sólida base en ingeniería. No obstante, en el ámbito clínico cada vez es más común la utilización de herramientas tecnológicas basadas en estos principios, especialmente en áreas como la rehabilitación y el control neuromuscular mediante EMG. Por ello, en el trabajo explorará la necesidad de traducir del funcionamiento del algoritmo a un lenguaje claro y práctico, facilitando su integración en entornos clínicos y garantizando un uso adecuado y seguro por parte de los especialistas.
...
En esta sección se expondrá la planificación del trabajo para su adecuada finalización. Se nombrarán los diferentes algoritmos de los que se dispone, pruebas que se realizarán y los procedimientos que sean pertinentes. Tras su explicación en detalle, se ilustrará con un cronograma.
...
\end{itemize}
Una vez separada las señales, es crucial reconocer e identificar cual de los componentes resultantes procede realmente de la señal de los nervios redirigidos frente a otros músculos. Para ello, se debe diseñar un procedimiento capaz de distinguir de manera fiable la fuente reinervada y emplear métricas, como SNR o correlación, para valorar los resultados.
\\

Ahora que se ha detallado la estructura a seguir, es crucial organizar tiempo y recursos. Para ello, la mejor estrategia es idear un cronograma que considere los pasos a seguir a lo largo del Trabajo Fin de Grado y su duración temporal estimada para poder garantizar un desarrollo progresivo y continuado. 

La figura que se muestra a continuación muestra el reparto temporal de las tareas del proyecto. Cabe destacar que esta organización es orientativa y puede sufrir cambios a medida que se avanza en el Trabajo Fin de Grado.
...
\begin{itemize}
    \item \textbf{GitHub}: Sistema de control de versiones utilizado para el almacenamiento, seguimiento, documentación y evolución del código desarrollado en Python y MATLAB. A continuación se comparte un enlace al repositorio donde se subirán y gestionarán todos los archivos referentes al Trabajo Fin de Grado: \href{https://github.com/acobianiregui/TFG}{acobianiregui/TFG}
